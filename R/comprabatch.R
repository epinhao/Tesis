...
text <- readLines("/Users/Emilio/Documents/workspace/Tesis/Bimbo/listing")

for(k in 1:length(text)) {

print(text[k])
out<-capture.output(text[k])
cat(out,file="out.txt",sep="\n",append=TRUE)

com <- readLines(paste("/Users/Emilio/Documents/workspace/Tesis/Bimbo/compra",text[k],".txt",sep=""))
ven <- readLines(paste("/Users/Emilio/Documents/workspace/Tesis/Bimbo/venta",text[k],".txt",sep=""))
test <- read.csv(paste("/Users/Emilio/Documents/workspace/Tesis/Bimbo/ordenes",text[k],".txt",sep=""), header=FALSE)
test2 <- read.csv(paste("/Users/Emilio/Documents/workspace/Tesis/Bimbo/ordenes2",text[k],".txt",sep=""), header=FALSE)
c <- list()
v <- list()
...
out<-capture.output(summary(res)$coefficients)
cat(out,file="out.txt",sep="\n",append=TRUE)
out<-capture.output(confint(res))
cat(out,file="out.txt",sep="\n",append=TRUE)
