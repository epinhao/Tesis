...
\usepackage{graphicx,setspace} % OS X
...
El algoritmo se implementó en Java y se desarrolló una interfaz gráfica. En la Figura \ref {algoritmo} se muestra un diagrama de flujo del algoritmo. Como se mencionó antes, el algoritmo carga la tabla de Posturas Iniciales, esto se marca con la función ``Read Tabla Iniciales'' en el diagrama de flujo. Mientras no se agoten las posturas del día en el registro, se determina el tipo de postura y se procede según corresponda. Para el caso de las Compras y Ventas, éstas se registran en la estructura correspondiente de la interfaz en las funciones ``Compra Add Row'' y ``Venta Add Row''. Para las cancelaciones, se localiza la postura a través de la función ``Find Order'' y se elimina con la función ``Remove Order''. Los hechos AH de la BMV se registran en la interfaz al llamar a ``Print Exec BMV''.\\

Para las compras, ventas y modificaciones es necesario verificar si las nuevas posturas perfeccionan un hecho. La primera condición es que éstos ocurran después de la apertura de la BMV a las 8:30am. Una vez que se revisa esto, se comprueba si el precio de la mejor postura de compra excede al de la mejor de venta; si esto sucede se perfecciona un hecho y sólo es necesario determinar el volumen del mismo.\\

Como se puede ver en la Figura \ref{java}, la interfaz permite analizar individualmente cada entrada del registro de una emisora para un día. La interfaz cuenta con 5 ventanas:
\begin{itemize}
...
\includegraphics[width=0.8\textwidth, trim=3.5cm 1cm 4cm 2cm]{flow.pdf}
...
Estas medidas se utilizarán también en las siguientes secciones para medir la bondad de ajuste para los volúmenes y las cancelaciones. Es importante mencionar que como en este caso los parámetros del modelo se estiman, los valores críticos de la prueba de Kolmogorov–Smirnov se tendrían que estimar a través de métodos de simulación Monte Carlo.\\

En la Figura \ref{fig:precios} se muestra el ajuste del modelo propuesto anteriormente contra los datos empíricos de la base de datos. Adicionalmente en la Figura \ref{fig:preciosqq} se pueden ver los QQ-Plots. Para AMX y COMERCI al ser emisoras con mayor liquidez, las órdenes llegan dentro de un rango más compacto. Como puede notarse en las gráficas, las medidas $D_n$ y $\bar{D}$ para BIMBO y COMERCI están por debajo de 0.10 y 0.05. En el caso de AMX los valores de las medidas $D_n$ y $\bar{D}$ exceden 0.20 y 0.15 respectivamente, por lo que se presume que el modelo no es adecuado. Las medidas $D_n$ y $\bar{D}$ también son relativamente altas para GRUMA por lo que también se presume que el modelo no es el óptimo para esta emisora. La distribución Lomax se concentra cerca del cero, por lo que la mayoría de las observaciones al menos en el caso de COMERCI y KUO parecen seguir esta distribución. Los punto lejanos se desvían, sin embargo, no parece haber un patrón claro, por lo que podría deberse a los reducidos tamaños de las muestras. Los resultados son muy interesantes por lo que en futuros trabajos se podría realizar un análisis más profundo del comportamiento de los precios.
...
En el caso de los volúmenes el ajuste para AMX es mejor que el observado en los precios, las medidas $D_n$ y $\bar{D}$ son menores; esto no es así para GRUMA y HERDEZ donde las medidas son mayores. De nuevo BIMBO y COMERCI presentan el mejor ajuste con medidas $D_n$ y $\bar{D}$ muy cercanas a 0.10 y 0.05 respectivamente, por lo que no se puede descartar que los volúmenes sigan una ley de potencias. En la Figura \ref{fig:volumen} se muestran algunas gráficas de la distribución, mientras que en la Figura \ref{fig:volumenqq} se muestran los QQ-Plots. Es interesante observar la preferencia de los participantes por volúmenes múltiplos de 1,000.
...
En la Figura \ref{fig:canc} y en los QQ-Plots de la Figura \ref{fig:cancqq}, la distribución Lomax no parece ser la más adecuada, ya que el grueso de las cancelaciones se concentra en el intervalo donde $\Delta<20$. En la mayoría de los casos las medidas $D_n$ y $\bar{D}$ exceden 0.10 y 0.20 por lo que se presume que la distribución Lomax no es la más adecuada para las cancelaciones.
...
Se compararon los resultados obtenidos con los análisis que se han realizado para otros mercados. Para algunas de las variables los resultados fueron similares a los encontrados en otros mercados; sin embargo, es difícil concluir a través de una prueba estadística que los precios siguen una ley de potencias. El volumen de las órdenes también se asemeja a una ley de potencias, sin embargo, las pruebas estadísticas tampoco son concluyentes. Las cancelaciones no tienen un comportamiento similar. A pesar de que las pruebas estadísticas no son concluyentes, se piensa que una ley de potencias puede ser un modelo apropiado, ya que en el caso de los precios, por ejemplo, los participantes interesados en comprar o vender un activo pondrán sus órdenes cerca del mejor precio para que éstas se ejecuten en un tiempo razonable.\\
...
Debido a las propiedades de la función de densidad $f$, se puede probar \linebreak \cite{abate1999} que $\hat{f}$ converge y está acotada entre la sucesión creciente de aproximantes pares $w_{2n}$ y la sucesión decreciente de aproximantes impares $w_{2n+1}$:
\[
